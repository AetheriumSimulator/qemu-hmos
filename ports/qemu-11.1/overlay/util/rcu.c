...
#ifndef CONFIG_HMOS_EMBEDDED_CORE
    /* Embedded sessions explicitly register/unregister their entry thread.
     * Registering the dlopen caller here would retain its TLS after it exits.
     * The process-lifetime call_rcu worker still registers itself normally.
     */
    rcu_register_thread();
#endif
}
